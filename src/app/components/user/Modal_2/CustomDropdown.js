...
const CustomDropdown = ({ options, value, onChange, isOpen, setIsOpen }) => {
  return (
    <div className="relative w-full">
      ...
      />
      {isOpen && (
        <div className="absolute top-full left-0 w-full mt-1 bg-white rounded-lg shadow-lg z-10">
          {options.map((option, index) => (
            ...
            >
              <input
                type="radio"
                name={`dropdown-${options[0]}`} // Ensures unique grouping for each dropdown
